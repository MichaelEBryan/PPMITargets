...
PD_B37, PD_B38 = 88_464_264, 87_997_920
...
def main():
    ...
    if len(hi):
        ...
        print("      aligned to the PD RISK allele (rs979812-T):")
    ...
    if len(cere):
        ...
        print(f"\n[4] colocalisation input: {len(mg)} variants shared between the "
              "PD scan and the cerebellum eQTL")
        if len(mg) > 20:
            ...
            print("    PP4 is the posterior for one shared causal variant; "
                  "PP3 for two distinct causal variants.")
    ...
    print(f"    significant in {len(n_by)} of 13 brain tissues; "
          "absent from substantia nigra, putamen, caudate, cortex")
