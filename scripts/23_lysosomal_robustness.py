#!/usr/bin/env python
"""
How robust is the lysosomal onset signal?

The competitive test says loci near lysosomal genes carry more onset signal than
the rest of the panel once risk signal is held fixed. Before that is worth
building on, it has to survive the obvious ways it could be an artefact:

  leave-one-out      is it driven by a single locus (GBA, or chr4p16.3)?
  size-matched null  is a set of this size unusual, or would any 12 loci do?
  definition         does it hold across independent lysosomal gene sets from
                     different databases?
  direction          are lysosomal risk alleles associated with earlier onset
                     specifically, or just with larger effects in either
                     direction?
  ancestry           does it hold in FinnGen, an independent population?
"""
...
def main():
    ...
    # alternative lysosomal definitions
    alt = {}
