#!/usr/bin/env python
"""
How often does stratify-and-compare-rankings get the answer wrong, and what
should be used instead?

Two estimators are compared on the same simulated data.

  naive     train a gradient-boosted model separately in each stratum, rank
            features by mean absolute SHAP, take the top k in each, and declare
            distinct architectures when the two lists do not intersect. This is
            the published procedure.

  formal    for each variant, test whether its effect differs between strata
            (a z-test on the difference of stratum-specific coefficients),
            and combine into one global heterogeneity statistic.

Four generative scenarios:

  null      identical causal variants and identical effect sizes in both strata
  dose      identical causal variants, effects scaled by a constant in one
            stratum -- the same architecture at a different dose
  partial   half the causal variants shared
  disjoint  no causal variant shared

Under `null` and `dose` the architectures are the same set of variants, so any
claim of distinctness is a false positive. Under `partial` and `disjoint` a
correct method should detect the difference. Reporting both error rates makes
the comparison a proper operating characteristic rather than a demonstration.

Run:  modal run 20_modal_method.py
"""
import modal, itertools, json, os
import paths as _P
...
def cell(cfg: dict):
    ...
    def formal(GA, yA, GB, yB):
        """Per-variant difference of stratum-specific log-odds, combined."""
        zs = []
        for jj in range(p):
            out = []
...
def main():
    ...
    out = str(_P.ROOT / "results/80_method_grid.json")
