#!/usr/bin/env python
"""
Gene-set analysis of onset age at locus level.

The gene-level sweep treats neighbouring genes as independent observations. They
are not: the APOE cluster contributes four genes carrying one signal, the APOA
cluster three. That inflates any gene set containing a gene family.

Here genes are merged into independent loci first. Genes whose windows fall
within `MERGE` of each other on the same chromosome become one locus, which
takes the best statistic among its members. A gene set is represented by the
loci containing at least one of its genes, and the competitive test runs over
loci. Set size is then counted in loci, not genes, so a four-gene cluster counts
once.

Because a locus can belong to several sets, and because collapsing changes the
effective number of tests, significance is assessed by permuting set membership
across loci rather than from the regression p-value.
"""
