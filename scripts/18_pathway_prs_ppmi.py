#!/usr/bin/env python
"""
Pathway-partitioned polygenic scores against true age at onset in PPMI.

Independent, individual-level test of the same question the locus-level analysis
asks from summary statistics. Each pathway score sums the participant's risk
alleles across that pathway's loci, weighted by the META5 risk effect, so the
score is the participant's polygenic burden in that mechanism specifically.

Scores are mutually adjusted: every pathway is tested with the other pathways in
the model, so what is reported is burden in that mechanism over and above overall
polygenic load.
"""
    ...
    for name, genes in p17.CURATED.items():
        ...
        for _, r in sel.iterrows():
            ...
            if g.notna().sum() < 100:
                continue
            g = g.fillna(g.mean())
            # dosage is of the panel effect allele; align to the risk-increasing allele
