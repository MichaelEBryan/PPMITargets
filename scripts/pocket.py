"""Grid-based cavity detection following LIGSITE.

A grid point clear of every atom is scored one protein-solvent-protein event
for each of seven scan directions along which protein is met in both
directions. Points with at least four events are retained and connected
components form pockets.
"""
import numpy as np
...
HYDROPHOBIC = {"ALA", "VAL", "LEU", "ILE", "MET", "PHE", "TRP", "PRO", "CYS",
               "TYR"}
# the seven LIGSITE scan directions: three axes and four cube diagonals
...
def _psp(occ, spacing, reach):
    """Count protein-solvent-protein events at every grid point."""
...
def find_pockets(xyz, resn, resid, elem, spacing=0.8, probe=1.4, reach=10.0,
                 ...
                 max_grid=90_000_000):
    """Return the largest pockets, each with its descriptors and grid points."""
    ...
    # mark occupied cells atom by atom, which is far cheaper than a full query
    ...
    for k in order:
        ...
        hyd = (np.mean([rn in HYDROPHOBIC for _, rn in lin])
               if lin else np.nan)
        # depth: distance from each pocket point to the nearest protein surface
        # atom, relative to the protein's own radius of gyration
