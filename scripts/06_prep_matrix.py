#!/usr/bin/env python
"""
Build a de-identified numeric matrix for the cloud experiments.

Drops PATNO, all dates, and all free-text. What ships is the feature matrix the
published pipeline actually used, plus the class label, the age stratum, and a
provenance tag per column. No participant identifiers leave this machine.
"""
import re, json, numpy as np, pandas as pd
import sys
sys.path.insert(0, str(_P.ROOT / "scripts"))
from importlib import import_module
import paths as _P
spec = import_module("02_leakage_audit") if False else None

SRC = str(_P.DATA / "Complete DataFrameC.csv")
OUTD = str(_P.ROOT / "data")
...
def main():
    ...
    # COHORT_x / COHORT_y are the numeric encoding of COHORT_DEFINITION. The
    # published notebook drops them; keeping them would make the task trivial.
