#!/usr/bin/env python
"""
Core age-at-onset analysis, now on the real onset variable.

Everything the earlier pass had to do with age at enrolment is redone here with
age at first motor symptom. Adds a survival framing, which is the right model
for an age-at-event outcome, and the case-case comparison on true onset strata.
"""
...
def core(d):
    """PD cases with onset age, European-inferred, genetic cohort removed."""
...
def sec_prs(c):
    ...
    # negative control in non-cases is not defined for AAO; use diagnostic delay instead
...
def sec_survival(c):
    """Age at onset as time to event. All cases have the event, so this is an
    accelerated-failure-time / Cox model on age, which handles the skew."""
