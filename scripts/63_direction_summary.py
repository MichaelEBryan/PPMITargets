#!/usr/bin/env python
"""Read the therapeutic direction off the aligned brain eQTLs.

For each gene, keep the expression signals that sit at its Parkinson's risk
locus: within 250 kb of the lead variant, and associated with disease within
two orders of magnitude of the lead itself, so that a weak locus is judged on
its own scale rather than against a fixed threshold.

Each brain region votes once, using the median slope of its own variants, so a
region with thousands of records cannot outvote the rest.
"""
