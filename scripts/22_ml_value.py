#!/usr/bin/env python
"""
Does nonlinear modelling of genotypes add anything over an additive score?

The original study's premise is that machine learning extracts something from
genotype data that conventional analysis misses. That is testable directly:
predict age at onset from the same variants using (a) an additive linear model,
(b) elastic net, (c) gradient boosting, and compare held-out accuracy under
nested cross-validation with the hyperparameter search inside the training folds.

A second arm screens explicitly for the thing boosting could in principle find
and regression could not: pairwise interaction between variants. Every pair is
tested and the best is compared against a permutation null built by shuffling
onset age, which preserves the genotype correlation structure exactly.
"""
...
def interaction_screen(G, y, cov):
    """Best pairwise genotype-by-genotype interaction vs a permutation null."""
    cols = list(G.columns)
...
def main():
    ...
    # paired test: does boosting beat the additive model fold-for-fold?
