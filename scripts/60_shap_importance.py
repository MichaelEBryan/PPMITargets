#!/usr/bin/env python
"""Shapley attributions for the four-class classifiers, with bootstrap ranks.

Fits the three gradient-boosted models per stratum on the corrected matrix,
records mean absolute Shapley value per feature, apportions the total to the
provenance blocks, and repeats on 200 bootstrap resamples to see how far the
ranking moves.
"""
...
def main():
    ...
    for sname, mask in strata.items():
        ...
        # point estimate of SHAP importance, per model
        per_model = {}
        ...
        # how far does the ranking move if the patients change?
        ...
        # SHAP dependence for the strongest genotype feature
        gmask = bl == "genotype"
        if gmask.any():
