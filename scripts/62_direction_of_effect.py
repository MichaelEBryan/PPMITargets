#!/usr/bin/env python
"""Align brain eQTL slopes onto the Parkinson's risk allele at each locus."""
...
def main():
    ...
    # pick the lead risk variant in a 500 kb window round each gene
    lead = {}
    for gene in GENES:
        ...
        if not len(w):
            ...
            continue
        b = w.loc[w.p.idxmin()]
        # orient onto the risk-increasing allele
    ...
    # every significant brain eQTL for these genes, aligned to risk
    # GTEx v8 is GRCh38 and the risk statistics are GRCh37, so each eQTL
    # variant is lifted back before the risk beta is read off at that position.
    ...
    for gene in GENES:
        ...
        L = lead.get(gene, {})
        # weight by how strongly the variant associates with disease risk
