#!/usr/bin/env python
"""
17q21.31: is 'LRRC37A2' a resolvable target, or a label on a haplotype?

Adds to the r^2 analysis: (a) |D'|, which is the right measure for asking whether
variants sit on the same haplotype background when allele frequencies differ;
(b) the gene each panel variant is assigned to by Ensembl, so we can see where a
'LRRC37A2' label would come from; (c) the identity of the best-tagging variants
and which genes they fall in.
"""
...
def ld_pair(hA, hB):
    """r^2 and |D'| from phased haplotypes (n_sample x 2 each)."""
...
def main():
    ...
    # LD of the LRRC37A2 variant with everything common in the window
    iA = idx["rs62053943"]
    r2s, dps = [], []
    for j in common:
