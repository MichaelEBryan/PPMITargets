...
def ed10():
    ...
    leader(ax4, f"all {u['n_pairs']:,} pairs", xy=(8.4, 0.10), xytext=(9.9, 0.42),
         color=WARM, ha="right")
    leader(ax4, f"{f_['n_pairs']:,} pairs with\nenough carriers",
         xy=(4.6, 0.42), xytext=(6.4, 0.72), color=NAVY)
    ax4.set_xlabel("strongest interaction found\nafter shuffling")
    ax4.set_ylabel("shuffles")
    ax4.set_xlim(2.5, 11); ax4.set_yticks([]); hide_y_spine(ax4)
    ...
    for a, s_ in zip([ax, ax2, ax3, ax4, ax5, ax6], "abcdef"):
        panel_label(fig, a, s_, x=a.get_position().x0 - 0.115)
    save(fig, "supp14_robustness_checks")


def ed11():
    CLAIMS = [
        ("Comparing ranked lists cannot tell shared from separate causes",
         ["pass", "pass", "pass", "na", "pass", "pass"]),
        ("How often it fires by chance depends on the list length chosen",
         ["pass", "na", "pass", "na", "na", "pass"]),
        ("A direct test of difference performs better",
         ["pass", "part", "pass", "na", "na", "part"]),
        ("Enrolment flags alone place 93% of participants correctly",
         ["pass", "na", "pass", "na", "na", "pass"]),
        ("The reported accuracy comes from study-design columns",
         ["pass", "na", "pass", "na", "na", "pass"]),
        ("Nonlinear models do no better than adding up risk alleles",
         ["pass", "na", "pass", "pass", "na", "part"]),
        ("Higher common-variant burden means earlier onset",
         ["pass", "na", "pass", "pass", "pass", "pass"]),
        ("...by 1.97 years per standard deviation specifically",
         ["fail", "na", "fail", "part", "fail", "na"]),
        ("Across 86 loci, risk alleles also bring onset forward",
         ["pass", "pass", "pass", "pass", "pass", "pass"]),
        ("Early and late onset differ variant by variant",
         ["fail", "na", "part", "fail", "na", "pass"]),
        ("Lysosomal loci carry extra signal on onset",
         ["fail", "na", "fail", "fail", "fail", "pass"]),
        ("No pathway acts on onset beyond its effect on risk",
         ["pass", "pass", "pass", "pass", "na", "pass"]),
        ("Raised GALC activity holds up on an independent test",
         ["pass", "pass", "na", "na", "pass", "pass"]),
        ("GALC and LRRC37A2 act on risk rather than on onset",
         ["pass", "pass", "pass", "na", "pass", "na"]),
    ]
    CHECKS = ["reproduces\nelsewhere", "holds when variants\nare correlated",
              "holds when one\nlocus is dropped", "holds after correcting\nfor many tests",
              "supported by\nexternal data", "the test is\nwell calibrated"]
...
def ed12():
    ...
    for a, s_ in zip(axes.ravel(), "abcd"):
        panel_label(fig, a, s_, x=a.get_position().x0 - 0.125)
    save(fig, "supp16_galc_structure")
...
def ed13():
    ...
    for a, s_ in zip([ax, ax2, ax3], "abc"):
        panel_label(fig, a, s_, x=a.get_position().x0 - 0.125)
    save(fig, "supp17_enrolment_proxy")
