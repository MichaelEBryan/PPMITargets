...
def ed2():
    ...
    for a, s in zip(axes, "ab"):
        panel_label(fig, a, s)
    save(fig, "supp07_false_positive_surface")
...
def ed3():
    ...
    leader(ax, "the two lists in the real data\nshared " +
    ...
    for a, s in zip([ax, ax2], "ab"):
        panel_label(fig, a, s)
    save(fig, "supp08_shared_feature_null")
...
def ed4():
    ...
    panel_label(fig, axes[0], "a", dy=0.040)
    panel_label(fig, axes[1], "b", x=axes[1].get_position().x0 - 0.032, dy=0.040)
    save(fig, "supp09_feature_ablation")
...
def ed5():
    ...
    top = ax.get_ylim()[1]
    leader(ax, "the strongest of the 6,259 gene sets\nin the real data",
         xy=(obs, top * 0.50), xytext=(3.30, top * 0.86), color=WARM)
    leader(ax, "the level a set would have had to reach\nto count as a finding",
    ...
    save(fig, "supp10_gene_sets_and_models")
...
def ed6():
    ...
    hide_y_spine(ax)
    save(fig, "supp11_variant_effect_auc")
...
def ed7():
    ...
    leader(axes[0], "rs62053943, in LRRC37A2", xy=(45.667, 1.02),
         xytext=(44.95, 1.09), color=NAVY)
    leader(axes[0], "rs17649553, in MAPT", xy=(45.917, 0.55),
    ...
    for a, s in zip(axes, "abc"):
        panel_label(fig, a, s)
    save(fig, "supp12_17q21_haplotype")
...
def ed8():
    ...
    for a, s in zip([ax, ax2], "ab"):
        panel_label(fig, a, s)
    save(fig, "supp13_recruitment_routes")
